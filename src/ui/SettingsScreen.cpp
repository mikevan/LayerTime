...
// Squachify needs art on the card. Probed here rather than cached so a
// card inserted after boot is picked up the next time Settings refreshes;
// this runs on page refresh, not on a render tick.
bool squachArtPresent()
{
    lv_fs_file_t file;
    if (lv_fs_open(&file, "A:/assets/squach.png", LV_FS_MODE_RD) != LV_FS_RES_OK) return false;
    lv_fs_close(&file);
    return true;
}
}
...
void SettingsScreen::buildMainPage()
{
    ...
    makeButton(_mainPage, "SQUACHIFY?", 25, 798, 250, 42, squachifyThunk);
    _squachifyValue = makeLabel(_mainPage, "OFF", 290, 810, 90, &lv_font_montserrat_20, Theme::teal());
    lv_obj_set_style_text_align(_squachifyValue, LV_TEXT_ALIGN_CENTER, 0);
...
void SettingsScreen::refreshMainValues()
{
    ...
    lv_label_set_text(_squachifyValue,
                      !squachArtPresent() ? "NO FILE"
                                           : (_settings->squachify ? "ON" : "OFF"));
}
...
void SettingsScreen::squachifyThunk(lv_event_t *event)
{
    auto *self = static_cast<SettingsScreen *>(lv_event_get_user_data(event));
    self->_settings->squachify = !self->_settings->squachify;
    self->refreshMainValues();
